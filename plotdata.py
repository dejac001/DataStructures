...
class PlotApplication(tkinter.Frame):
    ...
    def buildWindow(self):
        ...
        def loadFile(filename=None):
            if filename is None:
                filename = tkinter.filedialog.askopenfilename(title="Select_a_Plot_File")
            ...
            screen.setworldcoordinates(minX - 0.2 * xSize, minY - 0.2 * ySize,
                                       maxX + 0.2 * xSize, maxY + 0.2 * ySize)
            ...
            for i in range(0, 101, 10):
                ...
                theTurtle.write(("%1." + str(yPlaces) + "f") % y, align='right', font=("Arial", 12, "normal"))
            ...
            for sequence in sequences:
                ...
                for dataPoint in dataPoints:
                    ...
                    x, y = float(attr['x'].value), float(attr['y'].value)
                    theTurtle.goto(x, y)
                    theTurtle.dot()
                screen.update()

        fileMenu.add_command(label="Load_Plot_Data...", command=loadFile)
...
def main(datafile):
    ...
    plotApp.mainloop()
    print("Program_Execution_Completed.")
